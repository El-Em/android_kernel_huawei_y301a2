...
int mdp4_mixer_info(int mixer_num, struct mdp_mixer_info *info)
{
	...
	/*qcom patch to sovle kernel panic issue*/
...
void mdp4_overlay_borderfill_stage_up(struct mdp4_overlay_pipe *pipe)
{
	...
	/*if bspipe = NULL, prevent kernel panic issue*/
	if(NULL == bspipe)
	{
		WARN(1,"In %s,fatal error,bspipe is NULL\n",__func__);
		return;
	}
	/*
...
struct mdp4_overlay_pipe *mdp4_overlay_pipe_alloc(int ptype, int mixer)
{
	...
	for (i = 0; i < OVERLAY_PIPE_MAX; i++) 
	{
		printk("in %s,pipe_used= %d,pipe_type= %d,pipe_num= %d,pipe_ndx= %d,\n",__func__,
			ctrl->plist[i].pipe_used,ctrl->plist[i].pipe_type,ctrl->plist[i].pipe_num,ctrl->plist[i].pipe_ndx);
	}
	#endif
	return NULL;
}
