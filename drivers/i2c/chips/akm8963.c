...
/*turn on the power*/	
	...
	gpio_config = GPIO_CFG(MSM_8930_COMPASS_RST_PIN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA);
	err = gpio_tlmm_config(gpio_config, GPIO_CFG_ENABLE);
	if (err) 
	{
		printk(KERN_ERR "%s: gpio_tlmm_config(145)=%d\n", __func__, err);
	}
	ret = gpio_request(MSM_8930_COMPASS_RST_PIN, "akm8963");
	gpio_direction_output(MSM_8930_COMPASS_RST_PIN, 0);
	mdelay(1);
	gpio_direction_output(MSM_8930_COMPASS_RST_PIN, 1);
	mdelay(1);
	ret = gpio_get_value(MSM_8930_COMPASS_RST_PIN);
	...
	/*DATA IRQ SET*/
	gpio_config = GPIO_CFG(MSM_8930_COMPASS_INT_PIN, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA);
	err = gpio_tlmm_config(gpio_config, GPIO_CFG_ENABLE);
	if (err) 
	{
		err = -EIO;
		printk(KERN_ERR "%s: gpio_tlmm_config(#%d)=%d\n", __func__, MSM_8930_COMPASS_INT_PIN, err);
		goto exit_check_dev_id;
	}
	if (gpio_request(MSM_8930_COMPASS_INT_PIN, "compass_int\n"))
