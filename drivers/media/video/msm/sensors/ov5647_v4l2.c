...
static int camera_module_id = 0x1234;
...
static struct msm_camera_i2c_reg_conf ov5647_prev_settings[] = {
	...
	{0x303c, 0x11},//PLL   
	{0x3821, 0x07}, //00 mirror on
	{0x3820, 0x41}, //47 flip off
	{0x3612, 0x59}, 
	{0x3618, 0x00}, 
	{0x380c, 0x09}, 
	{0x380d, 0x70}, 
...
static struct msm_camera_i2c_reg_conf ov5647_snap_settings[] = {
	...
	{0x3036, 0x66},
	{0x303c, 0x11},
	{0x3821, 0x06}, //00 mirror on
	{0x3820, 0x00}, //47 flip off
	{0x3612, 0x5b},
	{0x3618, 0x04},
	{0x380c, 0x0a},
	{0x380d, 0xc0},
...
static struct msm_camera_i2c_reg_conf ov5647_prev_mirror_and_flip_settings[] = {
	/*1280*960 Reference Setting 24M MCLK 2lane 280Mbps/lane 30fps
	for back to preview*/
	{0x3035, 0x21},
	{0x3036, 0x66},//PLL,  0x64
	{0x303c, 0x11},//PLL   
	{0x3821, 0x01}, //00 mirror on
	{0x3820, 0x47}, //47 flip off
	{0x3612, 0x59}, 
	{0x3618, 0x00}, 
	{0x380c, 0x09}, 
	{0x380d, 0x70}, 
...
static struct msm_camera_i2c_reg_conf ov5647_snap_mirror_and_flip_settings[] = {
	/*2608*1952 Reference Setting 24M MCLK 2lane 280Mbps/lane 30fps*/
	{0x3208, 0x02},
	{0x3035, 0x21},
	{0x3036, 0x66},
	{0x303c, 0x11},
	{0x3821, 0x00}, //00 mirror on
	{0x3820, 0x06}, //47 flip off
	{0x3612, 0x5b},
	{0x3618, 0x04},
	{0x380c, 0x0a},
	{0x380d, 0xc0},
...
static struct msm_camera_i2c_reg_conf ov5647_zsl_mirror_and_flip_settings[] = {
	{0x3208, 0x02},
	{0x3035, 0x21},
	{0x3036, 0x66},
	{0x303c, 0x11},
...
static struct msm_camera_i2c_conf_array ov5647_mirror_and_flip_confs[] = {
	{&ov5647_snap_mirror_and_flip_settings[0],
	ARRAY_SIZE(ov5647_snap_mirror_and_flip_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&ov5647_prev_mirror_and_flip_settings[0],
	ARRAY_SIZE(ov5647_prev_mirror_and_flip_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&ov5647_video_60fps_settings[0],
	ARRAY_SIZE(ov5647_video_60fps_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&ov5647_video_90fps_settings[0],
	ARRAY_SIZE(ov5647_video_90fps_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&ov5647_zsl_mirror_and_flip_settings[0],
	ARRAY_SIZE(ov5647_zsl_mirror_and_flip_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
};
...
static struct msm_camera_csid_vc_cfg ov5647_cid_cfg[] = {
	{0, CSI_RAW10, CSI_DECODE_10BIT},
	{1, CSI_EMBED_DATA, CSI_DECODE_10BIT},
};

static struct msm_camera_csi2_params ov5647_csi_params = {
	.csid_params = {
		.lane_assign = 0xe4,
		.lane_cnt = 2,
		.lut_params = {
			.num_cid = ARRAY_SIZE(ov5647_cid_cfg),
			.vc_cfg = ov5647_cid_cfg,
		},
	},
	.csiphy_params = {
		.lane_cnt = 2,
...
static struct msm_camera_csi2_params *ov5647_csi_params_array[] = {
	&ov5647_csi_params,
	&ov5647_csi_params,
	&ov5647_csi_params,
	&ov5647_csi_params,
...
int RG_Ratio_Typical = 0x29A;
int BG_Ratio_Typical = 0x2D5;
		...
		const struct i2c_device_id *id)
{
    ...
    //modify msm_sensor_ctrl_t *s_ctrl to msm_camera_sensor_info *s_info
	...
	if(CAMERA_FLIP_AND_MIRROR == get_camera_mount_type(CAMERA_IS_MAIN))
	{
		printk("ov5647: camera need to be mirror and flip !\n");
		ov5647_regs.mode_settings = &ov5647_mirror_and_flip_confs[0];
		ov5647_regs.num_conf = ARRAY_SIZE(ov5647_mirror_and_flip_confs);
	}
    get_camera_sensor_name(board_name, MAX_SENSOR_NAME);
	if (0 == strcmp(board_name, "y340"))
		s_info->sensor_name="ov5647_y340";
	else
		s_info->sensor_name="ov5647";
		
	printk("%s: sensor name =%s\n", __func__, s_info->sensor_name);
	return rc;
}
...
int32_t ov5647_sensor_match_module(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	camera_module_id = OV5647_read_i2c(0x3d05 + otp_index*13 + 3);
	OV5647_write_i2c(0x3d21, 0x00);

	// clear otp buffer
	OV5647_write_i2c(0x3d05 + otp_index*13 + 3, 0x00);

	printk("%s:camera_module_id=0x%x\n", __func__, camera_module_id);	
	if((OV_SUNNY_AF == camera_module_id)||(OV_SUNNY_AF_0110 == camera_module_id))
	{
		s_ctrl->sensor_name = "23060110FA-OV-S";
		RG_Ratio_Typical = 0x29A;
		BG_Ratio_Typical = 0x2D5;
	}
	else if(OV_SUNNY_FF == camera_module_id)
	{
		s_ctrl->sensor_name = "23060084FF-OV-S";
...
//	.sensor_power_down = ov5647_sensor_power_down,
	...
	.sensor_match_module = ov5647_sensor_match_module,
};
static struct msm_sensor_reg_t ov5647_regs = {
...
static struct msm_sensor_ctrl_t ov5647_s_ctrl = {
	...
	//.csi_params = &ov5647_csi_params_array[0],
	.power_seq_delay = 20,
