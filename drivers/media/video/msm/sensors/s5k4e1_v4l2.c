...
#define MSB                             1
#define LSB                             0
#define S5k4E1_CAMERA_ID_GPIO  55

#define ADDR_FLIP_AND_MIRROR 0x0101
#define VALUE_FLIP_AND_MIRROR   0x03
#define VALUE_DEFAULT 0x00

static int s5k4e1_mirror_and_flip = false;

enum S5k4E1_MODULE_TYPE
{
	S5k4E1_MODULE_LITEON,
	S5k4E1_MODULE_SAMSUNG,
};

static enum S5k4E1_MODULE_TYPE s5k4e1_module_type = S5k4E1_MODULE_SAMSUNG;
...
static int32_t s5k4e1_sensor_match_module(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(!gpio_request(S5k4E1_CAMERA_ID_GPIO,  "s5k4e1"))
	{
		...
		{
			s5k4e1_module_type = S5k4E1_MODULE_SAMSUNG;
		}
		else
		{
			s5k4e1_module_type = S5k4E1_MODULE_LITEON;
		}
    ...
    if(s5k4e1_module_type == S5k4E1_MODULE_SAMSUNG)
    {
       	   s_ctrl->sensor_name="23060110FA-SAM-3";
    }
    else
    {
          s_ctrl->sensor_name="23060069FA-SAM-L";
    }
	...
	const struct i2c_device_id *id)
{
	...
	if(CAMERA_FLIP_AND_MIRROR == get_camera_mount_type(CAMERA_IS_MAIN)){
		s5k4e1_mirror_and_flip = true;
		//s_info->sensor_name="s5k4e1_g526";
		}
	get_camera_sensor_name(board_name, MAX_SENSOR_NAME);
	if (0 == strcmp(board_name, "g526"))
		s_info->sensor_name="s5k4e1_g526";
	else if (0 == strcmp(board_name, "y301_a2"))
		s_info->sensor_name="s5k4e1_y301_a2";
	else if (0 == strcmp(board_name, "y340"))
		s_info->sensor_name="s5k4e1_y340";
	else
		s_info->sensor_name="s5k4e1";
...
int32_t s5k4e1_write_init_settings(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc=0, i;
	...
	if(true == s5k4e1_mirror_and_flip)
	{
		printk("s5k4e1: camera need to be mirror and flip !\n");
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			ADDR_FLIP_AND_MIRROR, VALUE_FLIP_AND_MIRROR, MSM_CAMERA_I2C_BYTE_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			0x3096, 0x01, MSM_CAMERA_I2C_BYTE_DATA);
	}
...
static struct msm_sensor_fn_t s5k4e1_func_tbl = {
	...
	.sensor_match_module = s5k4e1_sensor_match_module,
	.sensor_write_init_settings = s5k4e1_write_init_settings,
};
