...
static int __devinit dev_dct_probe(struct platform_device *pdev)
{
	...
	for(i = 0;i<DEV_I2C_MAX;i++)
	{
		for(j = 0;j<sizeof(hw_dec_device_array)/sizeof(hw_dec_struct);j++)
		{
			...
			{
				value = get_I2C_devices(hw_dec_device_array[j].devices_name);
				if(!value || (value == -1))
				{
			...
			/*origin design is imprecision,the below lines will not work,
				keep it just for making sure it's same as shanghai's scheme*/
			if(j == (sizeof(hw_dec_device_array)/sizeof(hw_dec_struct)-1))
			{
				/*set the max bit to 1*/
				dev_flag_long = dev_flag_long | (1 << i);
			}
		}
	}
...
MODULE_AUTHOR("zhangmin777@huawei.com");
