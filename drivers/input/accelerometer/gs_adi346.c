...
static int gs_config_int_pin(void)
{
	...
	err = gpio_request(MSM_8930_GS_INT_1, "gpio_gs_int");
	if (err) 
	{
	...
	err = gpio_configure(MSM_8930_GS_INT_1, GPIOF_INPUT | IRQF_TRIGGER_HIGH);
	if (err) 
	{
		gpio_free(MSM_8930_GS_INT_1);
...
/*address has been configed in board-8930.c*/
