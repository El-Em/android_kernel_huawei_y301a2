...
/* Name: Synaptics upgrade FW , Notes if new FW , need to change file name */
...
/* delete the function syn_fw_update() because it is same to i2c_update_firmware() */

/*this structure is used to control TP update process*/
static struct {
	struct work_struct update_task;
	struct completion waiting_update;
...
/* move the variable into the struct syn_version_config */
...
static void synaptics_rmi4_work_func(struct work_struct *work)
{
	...
	/* workqueue mode will use these codes */
	#if	USE_IRQ_THREAD 
	#else
	if (ts->use_irq)
	{
...
irqreturn_t synaptics_rmi4_irq_handler(int irq, void *dev_id)
{
	...
	/* workqueue mode and thread_interrupt mode use different codes */
	#if	USE_IRQ_THREAD
	synaptics_rmi4_work_func(&ts->work);
	#else		
...
//to be improved .......
int RMI4_wait_attn(struct i2c_client * client,int udelay)
{
	int loop_count=0;
	int ret=0;
...
int RMI4_wait_for_idle(struct i2c_client * client,int udelay)
{
	int loop_count=0;
	int ret=0;
	...
	do{
		mdelay(udelay);
		/* Clear the attention assertion by reading the interrupt status register */
		ret = i2c_smbus_read_byte_data(client,fd_01.dataBase+1);//read the irq Interrupt Status
	}while(loop_count++ < 0x10 && ret <= 0);

	if(loop_count >= 0x10)
	{
		SYNAPITICS_DEBUG("RMI4 wait for idle timeout:ret=0x%x\n",ret);
		return -1;
	}
	return 0;
}
...
static int program_config(struct synaptics_rmi4 *ts, uint8_t *config)
{
	...
	for (i = 0; i < 0x20; i++) {
		...
		/*change the time of updated config */
...
static int syn_write_lockdown(struct synaptics_rmi4 *ts, uint8_t *config)
{
	...
	TS_DEBUG_RMI("Write device locked successfully.\n");	
	return 0;
}	
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		ret = touch_pdata->touch_power(IC_PM_ON);
		if(ret)
		{
	...
	#endif
#ifdef CONFIG_SYNAPTICS_UPDATE_RMI_TS_FIRMWARE
	atomic_set(&update_info.is_updating, 0);
	init_completion(&update_info.waiting_update);
	INIT_WORK(&update_info.update_task, update_thread);
#endif
/* delete one line, move to other place */
	g_ts->client = client;
...
/* move to get_ic_name()*/
	...
	if (g_ts->hasF11)
	{
		point_supported_huawei = get_tp_point_supported();
		if((point_supported_huawei > g_ts->f11.points_supported) || (point_supported_huawei <= 0)) 
		{
			point_supported_huawei = g_ts->f11.points_supported;
			TS_DEBUG_RMI("%s:outer config error, TP support %d points at most!\n",__func__,g_ts->f11.points_supported);
		}
		else
		{
			TS_DEBUG_RMI("synaptics tp support %d points!\n",point_supported_huawei);
		}
		g_ts->is_support_multi_touch = (point_supported_huawei >= 2) ? true : false;
	...
	if (client->irq)
	{
		...
		ret = request_threaded_irq(client->irq, NULL,synaptics_rmi4_irq_handler,
				IRQF_TRIGGER_LOW | IRQF_ONESHOT, client->name, g_ts);
		#else 
		ret = request_irq(client->irq, synaptics_rmi4_irq_handler,
				IRQF_TRIGGER_LOW, client->name, g_ts);
		#endif
		if(ret >= 0)
		{
...
err_check_functionality_failed:
	touch_pdata->touch_power(IC_PM_OFF);
...
static int RMI4_write_image(struct i2c_client *client,unsigned char type_cmd,const unsigned char *pgm_data)
{
	...
	{
		configdata[0] = fd_34.dataBase+2;
		for(i=0;i<block_size;i++)
		{
			configdata[i+1] = *(p_data+i);
		}
		ret = i2c_master_send(client, configdata, 17);
		if (ret < 0)
		{
...
static void update_thread(struct work_struct *work)
{
	...
	printk("The id of synaptics device is : %d\n",ret);
	...
	if( 0 != ret )
	{
		update_info.is_update_success = false;
		printk("Update firmware failed!\n");
	}
	else
	{
		update_info.is_update_success = true;
		printk("Update firmware success!\n");
		arm_pm_restart(0,&ret);
	}
	complete(&update_info.waiting_update);
}
/*
...
static ssize_t update_firmware_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if ( (buf[0] == '2')&&(buf[1] == '\0') )
	{
		/*if this function is called a second time during TP updating, it should do nothing and return*/
		if(!atomic_read(&update_info.is_updating))
		{
			atomic_set(&update_info.is_updating, 1);
			schedule_work(&update_info.update_task);
			wait_for_completion(&update_info.waiting_update);
			atomic_set(&update_info.is_updating, 0);
			if(update_info.is_update_success)
			{
				ret = 1;
			}
			else
		...
		{
			ret = -1;
			printk(KERN_WARNING "TP is updating,please wait!\n");
		}
	}
	else
	{
