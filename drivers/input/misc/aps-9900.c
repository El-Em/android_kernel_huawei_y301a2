...
/* the two parameters are in /sys/module/aps_9900/parameters */
static int apds_9900_pwindows_value = 200;
static int apds_9900_pwave_value = 100; 
module_param_named(apds_9900_pwindows_value, apds_9900_pwindows_value, int, S_IRUGO | S_IWUSR | S_IWGRP);
module_param_named(apds_9900_pwave_value, apds_9900_pwave_value, int, S_IRUGO | S_IWUSR | S_IWGRP);
        ...
        if (pdata >= pthreshold_h) 
        {
            ...
            if(atomic_read(&l_flag))
            {
                if (( irdata > sunlight ))
                {
                    ...
                    if (ret)
                    {
                        printk(KERN_ERR "set_9900_register is error(%d),clear failed!\n",ret);
                    }
                    else
                    {
                        printk("the sunlight is so strong that the proximity is influence, we filtrate it");
                    }
                    goto Light_init;
                }
            }
    ...
    struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    apds_9900_pwindows_value = get_proximity_window();
    apds_9900_pwave_value = get_proximity_wave(); 
    sunlight = get_proximity_sunlight();
    get_light_value(p);
    for(i = 0;i < ARRAY_SIZE(lsensor_adc_table) ; i++ )
    {
