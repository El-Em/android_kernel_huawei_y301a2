			   ...
			   const struct i2c_device_id *id)
{
    ...
    if (!platform_data->clock_output_ctrl) 
    { 
        printk(" <NFC> %s:clock_output_ctrl  missing\n",__func__); 
    } 
    else
    {
        // 0 - close for clock pin request mode, 1 - set for clock pin request mode
        ret = platform_data->clock_output_ctrl(1);
        printk(" <NFC> %s:clock_output_ctrl: ret = %d  \n",__func__,ret );
    }
    ...
    if (!platform_data->ven_cfg) 
    { 
        printk(" <NFC> %s:ven_cfg  missing\n",__func__); 
    } 
    else
    {
        ret = platform_data->ven_cfg();
        printk(" <NFC> %s:ven_cfg: ret = %d  \n",__func__,ret );
    }
...
{
    nfc_support_type nfc_support_feature = NO_SUPPORT ;
    nfc_chipset_type nfc_device_type = NONE;
        
    /* get nfc_support_feature from the device feature configuration (.xml file) */
    nfc_support_feature = get_nfc_support();
    
    /* get nfc chip type*/
    nfc_device_type = get_nfc_chipset_type();	
    
    if(SUPPORT != nfc_support_feature || BCM != nfc_device_type)
    {
        printk(KERN_ERR "%s:Current nfc is not BCM:%d-%d\n",__func__,nfc_support_feature,nfc_device_type);
        return 0;
    }
    printk(KERN_ERR "%s:Current nfc is BCM.\n",__func__);
