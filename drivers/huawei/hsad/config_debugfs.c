...
int  config_debugfs_init(void)
{
	...
	msm_gpio_curr_file = debugfs_create_file("curr_config",S_IFREG | S_IWUGO,msmgpio_debugfs_root, (void *) "curr_config",&msm_gpio_curr_cfg_fops);

	pm_gpio_init_file = debugfs_create_file("init_config",0444,pmgpio_debugfs_root,NULL,&pm_gpio_init_cfg_fops);
#endif

#ifdef CONFIG_HUAWEI_SUSPEND_GPIO
	msm_gpio_sleep_file = debugfs_create_file("sleep_config",S_IFREG | S_IWUGO,msmgpio_debugfs_root, (void *) "sleep_config",&msm_gpio_sleep_cfg_fops);
#endif	
	destroy_dentry_list();     
