...
{  /* machine_arch_type        s_board_id      hw_version_id */
   ...
   {MACH_TYPE_MSM8X25_U8825,"MSM8X25_U8825","HD1U8825M "},
...
{
/*app_info only add sensors_list and  compass_gs_position*/
#if 0
	int len;
   ...
   sprintf(hw_version_sub_ver, "VER.%c", 'A'+(char)get_hw_sub_board_id());
	...
	//char audio_property[AUDIO_PROPERTY_LEN] = {0};
	char * touch_info = NULL;
	char lcd_name[LCD_NAME_LEN] = {0};
	char battery_name[16] = {0};
    char board_name[BOARD_NAME_LEN] = {0};
    /* Read the board name of the phone. */
    ret = get_product_type(board_name, sizeof(board_name));
    if ( -1 == ret )
    {
        strncpy(board_name,"unknown name",sizeof(board_name));
    }
	ret = get_lcd_name(lcd_name,sizeof(lcd_name));
	if (ret < 0)
	{
		strncpy(lcd_name,"UNKNOWN LCD",sizeof(lcd_name));
	}
    touch_info = get_touch_info();
	if (touch_info == NULL)
	{
	...
	ret = get_batt_name_external(battery_name);
	if (ret < 0)
	{
		strncpy(battery_name,"Unknown battery",sizeof(battery_name));
	}
    ...
    "board_id:\n%d\n"
    "board_name:\n%s\n",
	lcd_name,sensors_list_name,compass_gs_name,battery_name,touch_info,g_current_board_id,board_name);
